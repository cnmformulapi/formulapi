...
# PID control values
Kp0 = 0.1	#0.05					# P term for d0 input (Offset)
Ki0 = 0.00	#0						# I term for d0 input (Offset)
Kd0 = 0.1	#.05					# D term for d0 input (Offset)
Kp1 = 0.055	#.03						# P term for d1 input (Angle)
Ki1 = 0.0	#0						# I term for d1 input (Angle)
Kd1 = 0.055	#0.03						# D term for d1 input (Angle)
Kp2 = 0.0	#0						# P term for d2 input (Curvature)
Ki2 = 0.00	#0						# I term for d2 input (Curvature)
Kd2 = 0.0	#0						# D term for d2 input (Curvature)
clipI = 40	#100		
# FIR filter settings
...
overtakeThreshold = 25				# Number of unexpected points before deciding there is a robot in front
overtakeLaneOffset = 2.0			# Lane shift away from the robot in front when overtaking
overtakeDurationSeconds = 3.0	#was 5.0	# Number of seconds to overtake for
overtakeBrakingSpeed = 0.5			# Speed percentage to slow down to when starting an overtake
overtakeBrakingSeconds = 1.5		# Number of seconds to slow down when overtaking


# Traffic light settings
lightsChangeThreshold = 44.0		# Minimum colour difference to flag as a change in light colour 
lightsRedGain = 2.0					# Gain term used to make the red and green levels consistent
lightsX1 = int(imageWidth  * 0.40)	# Lower X boundary for light detection
lightsX2 = int(imageWidth  * 0.60)	# Upper X boundary for light detection
...
# Work out the absolute distance from the start to each waypoint for lap 1
waypointDistances = []
distanceSoFar = 0.0
...
# Work out the absolute distance from the start to each waypoint for all other laps
waypointDistancesl1 = []
distanceSoFarl1 = 0.0
for i in range(len(distanceBetweenWaypointsl1) - 1):
	distanceSoFarl1 += distanceBetweenWaypointsl1[i]
	waypointDistancesl1.append(distanceSoFarl1)
del distanceSoFarl1
