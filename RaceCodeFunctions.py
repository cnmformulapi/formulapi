...
def WaitForWaypoint(pointNumber):
	...
	if Globals.running:
		if pointNumber < 1:
			...
			index = pointNumber - 2
			if LapCount() < 1:
                            distance = Settings.waypointDistancesl1[index]
                        else:
...
def LapCount():
        LogUserCall(None, None)
	if Globals.running:
                LogUserCall('%.1f', (Globals.lapCount))
...
def GetDistance():
	LogUserCall(None, None)
	if Globals.running:
                LogUserCall('%.3f', (Globals.lapTravelled))
...
def CurrentTrackPosition():
	LogUserCall(None, None)
	LogUserCall('%.3f', (Globals.controller.lastD0))
	return Globals.controller.lastD0

def CurrentAngle():
	LogUserCall(None, None)
	radians = math.atan(Globals.controller.lastD1)
	LogUserCall('%.3f', (radians * RAD_TO_DEG))
	return radians * RAD_TO_DEG

def TrackCurve():
	LogUserCall(None, None)
	LogUserCall('%.3f', (Globals.controller.lastD2))
